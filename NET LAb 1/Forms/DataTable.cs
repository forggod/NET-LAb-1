                ...
                switch (this.table)
                {
                    case "customer_orders":
                        query = @$"SELECT `a.id` as id, `date_order`, `b.name` as name, `total`, `status` 
                            ...
                            SELECT `a.id` as id, `a.id_order` as id_order, `name`, `quantity`, `total`, `remaining` 
...
;
        ...
        public void selectorDGV()
        {
            ...
            try
            {
                ...
                foreach (var row in resultSet.Rows)
                {
                    if (row["id_order"].GetOptionalUint64() == id)
                    {
        ...
        private void ToolStripMenuItem_report_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < dataGridView2.Rows.Count; i++)
                {
                    ...
                    {
                        worksheet.Cells[5 + i, j + 1] = dataGridView2.Rows[i].Cells[j].Value.ToString();
                    }
                }
